...
public class SocketServer : Socket
{
    ...
    public string Request(Socket handler)
    {
        if (handler != null) return Receive(handler);
        return string.Empty;
    }

    public void Responce(Socket handler, string? message)
    {
        if (message != null) Send(handler, message);
    }
